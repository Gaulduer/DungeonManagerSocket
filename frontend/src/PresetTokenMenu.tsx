...
export default function PresetTokenMenu(props: PresetMenuProps) {
  ...
  return <div style={{display: 'flex', position: 'fixed', zIndex: 2, left: 0, top: 0, width: '100%', height: '100%', alignItems: 'center', justifyContent: 'center', color: '#000000'}} >
    <div style={{zIndex: 10, backgroundColor: '#CCCCCC', padding: '100px', display: 'flex', flexDirection: 'column', alignItems: 'center'}} >
        <div style={{width: '100px', height: '100px'}}>
          <Token placement={{x: -1, y: -1, id: -1, type: 'token', contentID: -1, content: JSON.stringify({backgroundImage: image === '' ? undefined:image, outline: outline})}}></Token>
        </div>
        <p>Image URL</p>
        ...
        <button onClick={() => {
          socket.emit('add-preset', {x: -1, y: -1, id: -1, type: 'token', contentID: -1, content: JSON.stringify({backgroundImage: image === '' ? undefined:image, outline: outline})})
